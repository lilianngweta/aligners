...
#SBATCH --output=logs/aligners_phi2_%A_%a.out
#SBATCH --array=0-2
#SBATCH --nodes=3
...
timeout 5h srun deepspeed train_aligners.py --modelpath "microsoft/phi-2" --model_outdir "phi2" --index $SLURM_ARRAY_TASK_ID
  if [[ $? == 124 ]]; then 

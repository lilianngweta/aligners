#!/bin/bash
#SBATCH --job-name=aligners
#SBATCH --output=logs/aligners_redpajama_%A_%a.out
#SBATCH --array=1
#SBATCH --nodes=3
...
timeout 5h srun deepspeed train_aligners.py --modelpath "togethercomputer/RedPajama-INCITE-Base-3B-v1" --model_outdir "redpajama" --index $SLURM_ARRAY_TASK_ID
  if [[ $? == 124 ]]; then 

...
#SBATCH --output=logs/aligners_pythia_%A_%a.out
#SBATCH --array=0-2
#SBATCH --nodes=1
...
timeout 5h srun deepspeed train_aligners.py --modelpath "EleutherAI/pythia-1.4b-deduped" --model_outdir "pythia" --index $SLURM_ARRAY_TASK_ID
  if [[ $? == 124 ]]; then 

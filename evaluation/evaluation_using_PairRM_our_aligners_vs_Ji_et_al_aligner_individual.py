...
def evaluate_with_pairRM(eval_set_size=None, aligner_model_name=None,
                             base_model=None,checkpoint_id=None, evaluator=None,batch_size=None,type=None,path_id=None,i=None):
    
    all_combined_dfs = []
    all_types = [type]
    for type in all_types:
        aligned_by_aligners_df = None
        if path_id == "synthetic":
            aligned_by_aligners_df = pd.read_csv("../generate-responses-for-eval/data_aligned_individual/"+type+"/"+aligner_model_name+
        ...
        aligned_by_pku_aligner_df = None
        if path_id == "synthetic":
            ...
            aligned_by_pku_aligner_df = pd.read_csv("../generate-responses-for-eval/data_aligned/"+path_id+"/pku_aligner/"+base_model+
                                                     "/dfs/final/"+path_id+"_"+base_model+"_aligned_data_using_pku_aligner_aligner.csv")
    ...
    results_path = None
    if path_id == "synthetic":
        results_path = "./results_individual/pku_vs_our_aligners/"+type+"/"+aligner_model_name+"/checkpoint-"+str(checkpoint_id)+"/"+base_model+"/"+evaluator+"/"

    else:
        results_path = "./results_individual/pku_vs_our_aligners/"+path_id+"/"+type+"/"+aligner_model_name+"/checkpoint-"+str(checkpoint_id)+"/"+base_model+"/"+evaluator+"/"
    ...
    results['win_rate'] = float(accuracy_using_scores)
...
if __name__ == "__main__":
    ...
    grid = list(product(path_ids, base_models, aligner_model_names, types, evaluator_, checkpoint_ids, eval_set_size_,batch_size_))
